    ...
    <html lang="en" suppressHydrationWarning>
      ...
      <body>
        <TanStackQueryProvider>
          <Header />
          {children}  {/* Child routes render here */}
          <Footer />
          <TanStackDevtools />
        </TanStackQueryProvider>
        <Scripts />
...
export function CommentsTable() {
  ...
  const { isLoading: isInitialLoading } = useCommentsDataLoader()
  ...
  // Get filtered comments based on tag using live query
  ...
  const table = useReactTable({
    data: filteredComments, // Pre-filtered data from live queries
    columns,
...
- Filtering by tag buttons (All, Positive, Negative, Neutral, Unrelated)
...
# Production stage runs the built app
FROM base AS production
ENV NODE_ENV=production
ENV PORT=3000
COPY --from=build /app/dashboard/.output ./dashboard/.output
...
- TanStack DB localStorage caching (zero API calls after initial load)
