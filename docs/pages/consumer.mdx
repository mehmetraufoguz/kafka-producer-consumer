...
#### Redis Deduplication
**Location:** `consumer/src/deduplication.service.ts`

```typescript
async isDuplicate(text: string): Promise<boolean> {
  const hash = createHash('sha256').update(text).digest('hex')
  
  const exists = await this.redis.get(`comment:${hash}`)
  
  if (!exists) {
    await this.redis.setex(`comment:${hash}`, 10800, Date.now()) // 3h TTL
    return false
  }
...
**Why 3 hours:**
- Balances memory usage vs duplicate window
...
**Why LRU:**
- Faster than Redis for hot data
- Limited size (100 entries)
- Least Recently Used eviction

**Combined Flow:**
```
1. Check LRU cache (in-memory, fast)
   ├─ Hit → Duplicate found, discard
   └─ Miss → Check Redis
       ├─ Hit → Duplicate found, discard
       └─ Miss → New comment, continue processing
```
...
**Entity:** `consumer/src/entities/comment.entity.ts`
...
- `source` (for platform analytics)

**Why these indexes:**
- commentId: Fast duplicate checks
- tag: Dashboard filters by sentiment
- processedAt: "Last hour" statistics
